...
- **Code review fixes (2026-02-26) :**
  - H1 : corrigé `return null` → `return@withContext null` dans `DiscoveryRoutingEngine.computeRoute` (erreur de compilation Kotlin dans lambda non-inline).
  - H2 : Dijkstra O(n²) → `PriorityQueue` pour complexité O((V+E) log V) ; chemin reconstruit via map `previous` (plus de stockage de path dans chaque State → fin du risque OOM).
  - H3 : test `computeRoute_twoPaths_favorsUnexploredWhenInTolerance` rendu réellement assertif (vérifie le passage par le nœud intermédiaire inexploré).
  - M1 : test `onRequestRoute_success_updatesRoute` corrigé — simulation GPS via `ShadowLocationManager` pour éviter `userLocation == null` en Robolectric.
  - M2 : `RoutingRequest` maintenant utilisé dans `MapViewModel.onRequestRoute()`.
  - M3 : `haversineMeters` extraite en `util/GeoUtils.kt` (partagée entre `GraphBuilder` et `DiscoveryRoutingEngineImpl`).
...
- app/src/main/java/com/parcoursparis/routing/DiscoveryRoutingEngine.kt (créé, modifié code review)
- app/src/main/java/com/parcoursparis/map/MapUiState.kt (modifié)
- app/src/main/java/com/parcoursparis/map/MapViewModel.kt (modifié, modifié code review)
...
- app/src/main/java/com/parcoursparis/util/GeoUtils.kt (créé code review)
...
- app/src/test/java/com/parcoursparis/routing/DiscoveryRoutingEngineTest.kt (créé, modifié code review)
- app/src/test/java/com/parcoursparis/map/MapViewModelTest.kt (modifié, modifié code review)
